...
tx.executeSql('create table if not exists login(nom TEXT Primary Key ,pwd TEXT )', []);
});
}
...
db.transaction(function(tx) {
tx.executeSql('insert into login (nom,pwd) values (?,?)',['rania.chebbi','123'],
function(tx) {
alert('insertion éffectuée')
});
tx.executeSql('insert into login (nom,pwd) values (?,?)',['hbib.dhaouadi','456']
);

tx.executeSql('insert into login (nom,pwd) values (?,?)',['safa.farhat','789']
);
});
